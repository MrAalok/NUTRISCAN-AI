...
import { Camera, Upload, Search, Zap, CheckCircle2, RefreshCw, Barcode, Sparkles, AlertCircle } from 'lucide-react';
...
export default function ScannerHero({ onSelectProduct, isScanning, setIsScanning }) {
  const [activeTab, setActiveTab] = useState('camera'); // 'camera' | 'barcodeInput' | 'upload' | 'search'
  ...
  // Popular Real Barcodes for 1-Click Verification
  ...
  const handleRealBarcodeFetch = async (code) => {
    ...
    if (realProduct) {
      ...
      setScanError(`Product barcode "${code}" not found in Open Food Facts global database. Try searching by name.`);
    }
  };
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (results.length === 0) {
      setScanError(`No products found for "${searchQuery}". Try typing another brand or item.`);
    }
  };
    ...
    <section className="relative pt-6 pb-10 px-4 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="text-center max-w-3xl mx-auto mb-8">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/[0.04] border border-white/10 text-xs text-[#FF4B82] mb-4 font-medium backdrop-blur-md">
          ...
          <span>Tech for Health Initiative • Real-Time Open Food Facts API</span>
        </div>
        ...
        <p className="text-slate-300 text-sm sm:text-base leading-relaxed font-light">
          "Let's make technology the guardian of our health." Live API barcode lookup for real ingredient & nutrient breakdown.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto glass-panel p-6 sm:p-8 relative overflow-hidden">
        ...
        {activeTab === 'barcodeInput' && (
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              💡 <span className="text-slate-300 font-medium">Tip:</span> You can type any food barcode number from India (FSSAI) or internationally to query real-world Open Food Facts database.
            </p>
          </div>
        )}
        ...
        <div className="mt-6 pt-5 border-t border-white/10">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center justify-between">
            <span>⚡ Test Live Barcode API Fetch:</span>
            <span className="text-[10px] font-normal text-slate-500">Fetches Live Open Food Facts API</span>
          </p>
            ...
            {REAL_PRESET_BARCODES.map((item) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1 overflow-hidden">
                  ...
                  <p className="text-[10px] text-slate-400 font-mono">Barcode: {item.code}</p>
                </div>
              </button>
            ))}
          </div>
